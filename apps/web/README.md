...
Browser client for an existing ProofShip daemon. The Cloudflare Worker serves the
TanStack Start application only; it does not start, proxy, or store credentials
for a daemon.

```sh
bun --filter @proofship/web dev
```
...
```sh
WAKU_DAEMON_TOKEN=replace-me cargo run -p proofship-daemon --bin proofship-daemon -- \
  --bind 127.0.0.1:34123 \
  --allow-origin http://localhost:3001
```

ProofShip Desktop can expose the daemon it manages from Settings → Daemon, where you
...
```sh
bun --filter @proofship/web build
bun --filter @proofship/web deploy
```
