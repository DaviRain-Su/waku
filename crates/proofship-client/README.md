...
`proofship-client` is the Rust client for `proofship-daemon`. It owns the authenticated
...
preview caches. It depends on `proofship-protocol`, never on `proofship-core`.
