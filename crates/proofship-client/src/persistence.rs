...
use proofship_protocol::computer_use::ComputerAppGrant;
use proofship_protocol::i18n::AppLanguage;
use proofship_protocol::identity;
use proofship_protocol::model::{AgentSession, FavoriteModel, Project, ProviderKind};
use proofship_protocol::theme::ThemePreference;

pub use proofship_protocol::persistence::{
    ...
    fn migrate_loaded(&mut self) {
        for session in &mut self.sessions {
            let checkpoint_totals_current = session.turns.iter().all(|turn| {
                turn.checkpoint
                    .as_ref()
                    .is_none_or(proofship_protocol::model::Checkpoint::totals_are_current)
            });
            let before = (
                session.turns.len(),
...
impl StateStore {
    pub fn default_path() -> PathBuf {
        if cfg!(debug_assertions) {
            ...
            identity::data_directory().join("app.db")
        }
    }
    ...
    pub fn load(&self) -> io::Result<PersistedState> {
            ...
            .map_err(to_io_error)?
        {
            ResponsePayload::TaskState {
                ...
                proofship_protocol::projectless::set_workspace_root(projectless_root);
...
pub fn read_remote_reference(
    ...
    let command = if proofship_protocol::blob::is_reference(reference) {
        Command::ReadBlob {
            reference: reference.to_owned(),
        }
    } else if reference.starts_with(proofship_protocol::attachments::ATTACHMENT_SCHEME) {
