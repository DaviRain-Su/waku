//! Provider backend and driver-event wire translation for `proofship-daemon`.
...
use proofship_protocol::provider_session::{ProviderSessionFork, ProviderSessionForkRequest};
use proofship_protocol::ship::{McpServer, McpTransport};
use proofship_protocol::web3::CreatedWallet;
...
impl WakuBackend {
    ...
    fn mcp_rows(&self) -> anyhow::Result<Vec<proofship_protocol::ship::McpServer>> {
...
fn migrate_projectless_state(
    ...
    for index in indices {
        ...
        let workspace = crate::projectless::migrate_workspace(&old_path).with_context(|| {
            format!(
                "could not move projectless workspace {} under ~/.proofship/projects",
                old_path.display()
            )
        })?;
        ...
        match request.command {
            ...
            Command::Start { options } => {
                ...
                std::thread::Builder::new()
                    .name(format!("proofship-daemon-events-{session_id}"))
                    .spawn(move || {
