...
fn configure_computer_use_command(command: &mut Command, config: Option<&CodexComputerUseConfig>) {
    if let Some(config) = config {
        command
            ...
            .arg("-c")
            .arg(format!(
                "mcp_servers.proofship_js_repl.command={}",
                config.repl
            ))
            .arg("-c")
            .arg("mcp_servers.proofship_js_repl.args=[]")
            .arg("-c")
            .arg(format!(
                "mcp_servers.proofship_js_repl.env.WAKU_COMPUTER_USE_SERVER={}",
                config.server
            ))
            .arg("-c")
            .arg(format!(
                "mcp_servers.proofship_js_repl.env.WAKU_COMPUTER_USE_PROCESS_DIRECTORY={}",
...
mod tests {
    ...
    fn computer_use_command_configuration_follows_the_setting() {
        ...
        assert!(
            enabled_arguments
                .iter()
                .any(|argument| argument.contains("mcp_servers.proofship_js_repl.command"))
        );
        assert!(
            enabled_arguments
                .iter()
                .any(|argument| { argument == "mcp_servers.proofship_js_repl.args=[]" })
        );
        assert!(
            enabled_arguments
                .iter()
