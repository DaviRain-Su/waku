...
//! daemon settings cross the RPC boundary and are persisted by `proofship-daemon`.
//!
...
impl StateStore {
    ...
    pub fn new(path: PathBuf) -> Self {
        ...
        let configuration_directory = identity::configuration_directory();
    ...
    /// Local database owner used inside `proofship-daemon`. It never reads or
...
mod tests {
    ...
    fn default_path_is_build_specific() {
        ...
        {
            assert!(
                directory == Some(std::ffi::OsStr::new(identity::DATA_DIRECTORY_NAME))
                    || directory
                        == Some(std::ffi::OsStr::new(identity::LEGACY_DATA_DIRECTORY_NAME))
            );
            let configuration_directory = identity::configuration_directory();
            assert_eq!(
