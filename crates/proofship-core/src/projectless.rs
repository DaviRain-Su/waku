...
//! random fallback. ProofShip mirrors that layout beneath `~/.proofship/projects` so
...
pub fn set_workspace_root(root: Option<PathBuf>) {
    proofship_protocol::projectless::set_workspace_root(root.clone());
...
pub fn create_workspace(prompt: Option<&str>) -> io::Result<Workspace> {
    let root = workspace_root().ok_or_else(|| {
        io::Error::new(
            ...
            "could not locate the home directory for ~/.proofship/projects",
        )
    })?;
