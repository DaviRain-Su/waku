...
impl RequestDispatcher {
    ...
    fn dispatch_independent(
        ...
        if let Err(error) = std::thread::Builder::new()
            .name("proofship-daemon-request".into())
            .spawn(move || {
    ...
    fn dispatch_runtime(
        ...
        loop {
            ...
            let worker = std::thread::Builder::new()
                .name(format!("proofship-daemon-runtime-{session_id}"))
                .spawn(move || {
                    run_runtime_mailbox(
                        session_id,
                        mailbox_id,
        ...
        match listener.accept() {
            Ok((stream, _)) => {
                ...
                std::thread::Builder::new()
                    .name("proofship-daemon-connection".into())
                    .spawn(move || {
                        let _connection_permit = connection_permit;
                        if let Err(error) =
                            handle_connection(stream, &token, dispatcher, hub, shutdown, &options)
                        {
                            eprintln!("proofship-daemon connection ended: {error:#}");
                        }
                    })
    ...
    'connection: while !shutdown.load(Ordering::Acquire) {
        ...
        match socket.read() {
            Ok(Message::Text(text)) => match serde_json::from_str(text.as_ref()) {
                ...
                Err(error) => {
                    eprintln!("proofship-daemon ignored invalid message: {error}");
                }
            },
...
mod tests {
    ...
    use proofship_client::DaemonClient;
    use serde_json::json;
