...
use chrono::Utc;
use proofship_protocol::ship::{HostingProvider, HostingRecord, HostingTokenStatus};
use uuid::Uuid;
...
impl HostingStore {
    ...
    pub fn deploy(&self, cwd: &Path, provider: HostingProvider) -> anyhow::Result<HostingRecord> {
        if !cwd.is_dir() {
        ...
        let url = extract_url(&stdout, &stderr)
            .ok_or_else(|| anyhow!("deploy finished but no URL was found in the CLI output"))?;
...
fn deploy_dir(detect: &proofship_protocol::ship::FrontendDetect) -> String {
