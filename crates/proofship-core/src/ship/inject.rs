...
const CLOUDFLARE_SKILL: &str =
    include_str!("../../../../resources/skills/proofship-cloudflare/SKILL.md");
const VERCEL_SKILL: &str = include_str!("../../../../resources/skills/proofship-vercel/SKILL.md");
    ...
    servers.iter().any(|server| {
        server.enabled
            && server.url.as_deref().is_some_and(|url| {
                url.contains("mcp.cloudflare.com")
                    || url.contains("bindings.mcp.cloudflare.com")
    ...
    servers.iter().any(|server| {
        server.enabled
            && server
                .url
                .as_deref()
                .is_some_and(|url| url.contains("mcp.vercel.com"))
    })
}
