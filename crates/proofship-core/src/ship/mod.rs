...
use proofship_protocol::ship::{McpServer, ShipHistoryItem};
use proofship_protocol::web3::{DeploymentRecord, EvmNetwork, explorer_address_url};
...
pub use preview::{
    detect_frontend, scan as scan_frontend, start as start_preview, status as preview_status,
    stop as stop_preview,
};
...
pub fn enabled_catalog() -> Vec<McpServer> {
    CATALOG.get().map(McpCatalog::enabled).unwrap_or_default()
}
...
mod tests {
    use super::*;
    use proofship_protocol::web3::DeploymentRecord;
    ...
    fn history_merges_contract_and_hosting_by_cwd() {
        ...
        stores
            .hosting
            .append(proofship_protocol::ship::HostingRecord {
                id: "h1".into(),
        ...
        let sepolia = proofship_protocol::web3::builtin_networks()
            .into_iter()
        ...
        let other = stores.history(Some(std::path::Path::new("/tmp/other")), &contracts, &[]);
