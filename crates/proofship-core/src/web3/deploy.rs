...
use super::local_wallet::{WalletSecrets, send_with_key, send_with_local};
...
const MAINNET_CHAIN_IDS: &[u64] = &[1, 10, 56, 137, 196, 8453, 42161, 43114];
    ...
    runtime.block_on(async {
        match wallet.source {
            ...
            WalletSource::DevEnvKey => {
                ...
                let sent =
                    send_with_key(key.trim(), &network.rpc_url, network.chain_id, to, data).await?;
                Ok(sent.tx_hash)
            }
            WalletSource::WalletConnect => {
                bail!("WalletConnect was removed — create or import a local key")
...
mod tests {
    use super::*;
    use proofship_protocol::web3::builtin_networks;
