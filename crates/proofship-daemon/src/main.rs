...
use proofship_protocol::{DAEMON_TOKEN_ENV, DaemonReady, PROTOCOL_VERSION};

fn main() -> anyhow::Result<()> {
    let arguments = Arguments::parse(std::env::args().skip(1))?;
    proofship_protocol::identity::migrate_legacy_data_directory();
    let token = std::env::var(DAEMON_TOKEN_ENV)
        .context("ProofShip daemon authentication token is missing")?;
    ...
    let listener = TcpListener::bind(&arguments.bind)
        .with_context(|| format!("could not bind ProofShip daemon to {}", arguments.bind))?;
    ...
    if let Some(parent_pid) = arguments.parent_pid {
        ...
        std::thread::Builder::new()
            .name("proofship-daemon-parent".into())
            .spawn(move || {
    ...
    let task_path = proofship_core::persistence::StateStore::default_path();
    let settings = proofship_core::DaemonSettingsStore::open_with_legacy(
        proofship_core::DaemonSettings::default_path(),
        [task_path.with_file_name("settings.json")],
    )
    .context("could not load daemon settings")?;
    let task_store = proofship_core::persistence::StateStore::daemon(task_path);
    proofship_core::serve(
        listener,
        token,
        Arc::new(proofship_core::daemon::WakuBackend::new(
            settings, task_store,
        )?),
        shutdown,
        proofship_core::ServerOptions {
