...
use waku_protocol::ship::{
    McpServer, McpTransport, builtin_mcp_servers, is_builtin_mcp_id,
};
...
mod tests {
    ...
    fn cannot_remove_or_edit_builtin() {
        ...
        assert!(servers.iter().any(|server| server.id == "github" && server.enabled));
    }
    ...
    fn custom_http_roundtrip() {
        ...
        let servers = store
            ...
            .unwrap();
        assert!(servers.iter().any(|server| server.name == "Docs" && server.enabled));
    }
}
