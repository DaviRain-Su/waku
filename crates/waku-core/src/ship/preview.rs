...
use waku_protocol::ship::{FrontendDetect, PreviewStatus};
...
pub fn status(cwd: &Path) -> PreviewStatus {
    ...
    let key = cwd_key(cwd);
    let guard = hub().inner.lock().unwrap_or_else(|error| error.into_inner());
...
pub fn stop(cwd: &Path) -> PreviewStatus {
    let key = cwd_key(cwd);
    let mut guard = hub().inner.lock().unwrap_or_else(|error| error.into_inner());
...
fn spawn_dev(cwd: &Path, port: u16, kind: &str) -> anyhow::Result<Child> {
    ...
    match kind {
        "next" => {
            command.args(["--yes", "next", "dev", "-p", &port.to_string(), "-H", "127.0.0.1"]);
        }
        _ => {
            command.args([
                "--yes",
