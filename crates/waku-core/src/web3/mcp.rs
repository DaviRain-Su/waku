...
const PF_MCP_BIN: &str = "waku-pf-mcp";
...
fn detect_pf_mcp() -> Option<PathBuf> {
    if !toolchain_signal() && env_path("WAKU_PF_MCP").is_none() && env_path("PROOFSHIP_PF_MCP").is_none()
    {
        return None;
    }
...
pub fn attach_codex_mcp(command: &mut Command) {
    ...
    if let Some(bin) = attachment.pf_mcp {
        ...
        for (key, value) in super::pf_cli_env() {
            command.arg("-c").arg(format!(
                "mcp_servers.waku_pf_mcp.env.{key}={value}"
            ));
        }
    }
    if let Some(key) = attachment.okx_key {
        command
            .arg("-c")
            .arg(format!("mcp_servers.{ONCHAINOS_MCP_NAME}.url={ONCHAINOS_MCP_URL}"))
            .arg("-c")
            .arg(format!(
...
pub fn merge_grok_mcp(mcp_servers: &mut toml::Table) {
    ...
    if let Some(key) = attachment.okx_key {
        ...
        headers.insert(
            ACCESS_KEY_HEADER.into(),
            toml::Value::String(key),
        );
        let mut server = toml::Table::new();
        server.insert(
            "url".into(),
            toml::Value::String(ONCHAINOS_MCP_URL.into()),
        );
        server.insert("http_headers".into(), toml::Value::Table(headers));
        mcp_servers.insert(ONCHAINOS_MCP_NAME.replace('-', "_"), toml::Value::Table(server));
    }
...
fn write_claude_mcp_config() -> Option<PathBuf> {
    ...
    if let Some(bin) = attachment.pf_mcp {
        ...
        mcp_servers.insert("waku-pf-mcp".into(), server);
    }
    if let Some(key) = attachment.okx_key {
        mcp_servers.insert(
