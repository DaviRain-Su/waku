...
use waku_protocol::web3::PfToolchainStatus;

pub const PINNED_VERSION: &str = "0.1.1";
const PINNED_URL_DARWIN_ARM64: &str =
    "https://github.com/DaviRain-Su/proof_forge/releases/download/v0.1.1/proof-forge-next-0.1.1-darwin-arm64.tar.gz";
...
fn state() -> &'static Mutex<InstallState> {
    STATE.get_or_init(|| Mutex::new(InstallState {
        installing: false,
        last: detect_status(),
    }))
}
    ...
    env_file("PF_CLI")
        ...
        .or_else(|| {
            env_dir("PROOF_FORGE_ROOT").and_then(|root| {
                ...
                built.is_file().then_some(built).or_else(|| cli_under(&root))
            })
        })
}
