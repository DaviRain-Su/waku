---
name: proofship-cloudflare
description: >-
  ProofShip's default Web2 half: Cloudflare Workers, Pages, R2, KV, D1.
...
# ProofShip Cloudflare (default Web2)

A gated contract is the on-chain kernel. Real products also need a frontend
and a Web2 half. **Cloudflare is that default** in ProofShip — toggle it in
Settings → MCP.

## When this is attached

Settings → MCP has the Cloudflare rows **on**. Then every new run carries
those HTTP URLs. The coding agent opens ProofShip's built-in browser the first
