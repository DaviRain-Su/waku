...
// the archive half and resources/windows/proofship.iss for the installer half.
//
// Usage:
...
const packageDirectoryName = `proofship-${version}-${targetTriple}`;
const archive = join(releaseDirectory, `${packageDirectoryName}.zip`);
const installer = join(
  releaseDirectory,
  `ProofShip-${version}-${architecture}-Setup.exe`,
);

await $`cargo build --locked --release --package proofship --bin proofship --package proofship-daemon --bin proofship-daemon --package proofship-pf-mcp --bin proofship-pf-mcp`;
...
try {
  ...
  for (const file of ["proofship.exe", "proofship-daemon.exe", "proofship-pf-mcp.exe"]) {
  ...
  if (certificateData && certificatePassword) {
    ...
    await sign(signtool, certificate, certificatePassword, [
      join(packageDirectory, "proofship.exe"),
      join(packageDirectory, "proofship-daemon.exe"),
      join(packageDirectory, "proofship-pf-mcp.exe"),
    ]);
  } else {
  ...
  await $`${findInnoSetupCompiler()} ${`/DAppVersion=${version}`} ${`/DArch=${architecture}`} ${`/DStageDir=${packageDirectory}`} ${`/DOutputDir=${releaseDirectory}`} ${join(projectRoot, "resources", "windows", "proofship.iss")}`;
