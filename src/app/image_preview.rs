...
impl Waku {
    ...
    pub(super) fn image_for_reference(
        ...
        let attachment_reference =
            reference.starts_with(proofship_protocol::attachments::ATTACHMENT_SCHEME);
        if !proofship_protocol::blob::is_reference(reference) && !attachment_reference {
            return None;
        }
        ...
        cx.spawn(async move |waku, cx| {
            let image = cx
                .background_executor()
                .spawn(async move {
                    proofship_client::persistence::read_remote_reference(
                        &fetch_reference,
