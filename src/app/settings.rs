...
impl Waku {
    ...
    fn daemon_exposure_from_fields(
        &self,
        cx: &App,
    ) -> Result<proofship_client::DaemonExposureSettings, String> {
        let port = self
            .daemon_port_input
            .read(cx)
            .content()
        ...
        settings
            ...
            .and_then(proofship_client::DaemonExposureSettings::validate)
    ...
    fn regenerate_daemon_token(&mut self, cx: &mut Context<Self>) {
        ...
        settings.token = proofship_client::DaemonExposureSettings::new_token();
        self.daemon_token_revealed = false;
        self.apply_daemon_exposure(settings, cx);
    }
    ...
    fn apply_daemon_exposure(
        &mut self,
        settings: proofship_client::DaemonExposureSettings,
        cx: &mut Context<Self>,
    ) {
    ...
    fn set_ui_font_size(&mut self, size: f32, window: &mut Window, cx: &mut Context<Self>) {
        let size = proofship_client::persistence::sanitized_ui_font_size(size);
    ...
    fn set_code_font_size(&mut self, size: f32, cx: &mut Context<Self>) {
        let size = proofship_client::persistence::sanitized_code_font_size(size);
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                let result = match daemon.request(
                    Uuid::nil(),
                    Uuid::nil(),
                    proofship_client::Command::ProbeComputerPermissions { prompt },
                ) {
                    Ok(proofship_client::ResponsePayload::ComputerPermissions { permissions }) => {
                        Ok(permissions)
                    }
