    ...
    fn start(deadline: Option<Instant>) -> anyhow::Result<Self> {
        ...
        connection.request(
            ...
            json!({
                "protocolVersion": MCP_PROTOCOL_VERSION,
                "capabilities": {},
                "clientInfo": {"name": "proofship_js_repl", "version": env!("CARGO_PKG_VERSION")}
            }),
            deadline,
        )?;
...
mod tests {
    ...
    fn tool_list_matches_the_supported_repl_surface() {
        ...
        assert!(SERVER_INSTRUCTIONS.contains("`proofship_js_repl`"));
